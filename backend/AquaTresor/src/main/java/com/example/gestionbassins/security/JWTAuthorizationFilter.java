...
public class JWTAuthorizationFilter  extends OncePerRequestFilter{
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
	    ...
	    if (jwt == null || !jwt.startsWith(SecParams.PREFIX)) {
	        System.out.println("No JWT token or invalid prefix");
	        filterChain.doFilter(request, response);
	        return;
	    }
	    ...
	    try {
	        JWTVerifier verifier = JWT.require(Algorithm.HMAC256(SecParams.SECRET)).build();
	        jwt = jwt.substring(SecParams.PREFIX.length()); // Remove the "Bearer " prefix
	        DecodedJWT decodedJWT = verifier.verify(jwt);

	        String username = decodedJWT.getSubject();
	        List<String> roles = decodedJWT.getClaims().get("roles").asList(String.class);

	        // Log the extracted username and roles
	        ...
	        System.out.println("Security context updated with user: " + username + ", roles: " + roles);
	    } catch (Exception e) {
