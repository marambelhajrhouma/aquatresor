...
function isDataSource(value) {
  return value && typeof value.connect === "function" && !(value instanceof import_rxjs6.ConnectableObservable);
}
var ArrayDataSource = class extends DataSource {
  ...
  connect() {
    return (0, import_rxjs6.isObservable)(this._data) ? this._data : (0, import_rxjs6.of)(this._data);
  }
  disconnect() {
  }
};
...
var FixedSizeVirtualScrollStrategy = class {
  ...
  constructor(itemSize, minBufferPx, maxBufferPx) {
    this._scrolledIndexChange = new import_rxjs7.Subject();
    this.scrolledIndexChange = this._scrolledIndexChange.pipe((0, import_operators5.distinctUntilChanged)());
    this._viewport = null;
...
var ScrollDispatcher = class _ScrollDispatcher {
  constructor(_ngZone, _platform, document2) {
    this._ngZone = _ngZone;
    this._platform = _platform;
    this._scrolled = new import_rxjs7.Subject();
  ...
  scrolled(auditTimeInMs = DEFAULT_SCROLL_TIME) {
    ...
    return new import_rxjs7.Observable((observer) => {
      ...
      const subscription = auditTimeInMs > 0 ? this._scrolled.pipe((0, import_operators5.auditTime)(auditTimeInMs)).subscribe(observer) : this._scrolled.subscribe(observer);
      this._scrolledCount++;
      return () => {
        subscription.unsubscribe();
        this._scrolledCount--;
  ...
  ancestorScrolled(elementOrElementRef, auditTimeInMs) {
    ...
    return this.scrolled(auditTimeInMs).pipe((0, import_operators5.filter)((target) => {
  ...
  _addGlobalListener() {
    this._globalSubscription = this._ngZone.runOutsideAngular(() => {
      const window2 = this._getWindow();
      return (0, import_rxjs7.fromEvent)(window2.document, "scroll").subscribe(() => this._scrolled.next());
    });
  }
...
var CdkScrollable = class _CdkScrollable {
  constructor(elementRef, scrollDispatcher, ngZone, dir) {
    ...
    this._elementScrolled = new import_rxjs7.Observable((observer) => this.ngZone.runOutsideAngular(() => (0, import_rxjs7.fromEvent)(this.elementRef.nativeElement, "scroll").pipe((0, import_operators5.takeUntil)(this._destroyed)).subscribe(observer)));
  }
  ngOnInit() {
...
var ViewportRuler = class _ViewportRuler {
  constructor(_platform, ngZone, document2) {
    this._platform = _platform;
    this._change = new import_rxjs7.Subject();
  ...
  change(throttleTime = DEFAULT_RESIZE_TIME) {
    return throttleTime > 0 ? this._change.pipe((0, import_operators5.auditTime)(throttleTime)) : this._change;
  }
  /** Use defaultView of injected document if available or fallback to global window reference */
  _getWindow() {
    return this._document.defaultView || window;
...
var SCROLL_SCHEDULER = typeof requestAnimationFrame !== "undefined" ? import_rxjs7.animationFrameScheduler : import_rxjs7.asapScheduler;
var CdkVirtualScrollViewport = class _CdkVirtualScrollViewport extends CdkVirtualScrollable {
  ...
  constructor(elementRef, _changeDetectorRef, ngZone, _scrollStrategy, dir, scrollDispatcher, viewportRuler, scrollable) {
    ...
    this._platform = inject(Platform);
    this._detachedSubject = new import_rxjs7.Subject();
    this._renderedRangeSubject = new import_rxjs7.Subject();
    this._orientation = "vertical";
    this.appendOnly = false;
    this.scrolledIndexChange = new import_rxjs7.Observable((observer) => this._scrollStrategy.scrolledIndexChange.subscribe((index) => Promise.resolve().then(() => this.ngZone.run(() => observer.next(index)))));
    ...
    this._viewportChanges = import_rxjs7.Subscription.EMPTY;
    this._injector = inject(Injector);
  ...
  ngOnInit() {
    ...
    this.ngZone.runOutsideAngular(() => Promise.resolve().then(() => {
      ...
      this.scrollable.elementScrolled().pipe(
        ...
        (0, import_operators5.startWith)(null),
        // Collect multiple events into one until the next animation frame. This way if
        // there are multiple scroll events in the same frame we only need to recheck
        // our layout once.
        (0, import_operators5.auditTime)(0, SCROLL_SCHEDULER),
  ...
  attach(forOf) {
    ...
    this.ngZone.runOutsideAngular(() => {
      this._forOf = forOf;
      this._forOf.dataStream.pipe((0, import_operators5.takeUntil)(this._detachedSubject)).subscribe((data) => {
...
var CdkVirtualForOf = class _CdkVirtualForOf {
  ...
  set cdkVirtualForOf(value) {
    ...
    if (isDataSource(value)) {
      ...
      this._dataSourceChanges.next(new ArrayDataSource((0, import_rxjs7.isObservable)(value) ? value : Array.from(value || [])));
    }
  }
  /**
  ...
  constructor(_viewContainerRef, _template, _differs, _viewRepeater, _viewport, ngZone) {
    ...
    this.viewChange = new import_rxjs7.Subject();
    this._dataSourceChanges = new import_rxjs7.Subject();
    this.dataStream = this._dataSourceChanges.pipe(
      // Start off with null `DataSource`.
      (0, import_operators5.startWith)(null),
      // Bundle up the previous and current data sources so we can work with both.
      (0, import_operators5.pairwise)(),
      // Use `_changeDataSource` to disconnect from the previous data source and connect to the
      // new one, passing back a stream of data changes which we run through `switchMap` to give
      // us a data stream that emits the latest data from whatever the current `DataSource` is.
      (0, import_operators5.switchMap)(([prev, cur]) => this._changeDataSource(prev, cur)),
      // Replay the last emitted data when someone subscribes.
      (0, import_operators5.shareReplay)(1)
    );
    this._differ = null;
    ...
    this._viewport.renderedRangeStream.pipe((0, import_operators5.takeUntil)(this._destroyed)).subscribe((range2) => {
  ...
  _changeDataSource(oldDs, newDs) {
    ...
    this._needsUpdate = true;
    return newDs ? newDs.connect(this) : (0, import_rxjs7.of)();
  }
...
var CdkVirtualScrollableWindow = class _CdkVirtualScrollableWindow extends CdkVirtualScrollable {
  constructor(scrollDispatcher, ngZone, dir) {
    ...
    this._elementScrolled = new import_rxjs7.Observable((observer) => this.ngZone.runOutsideAngular(() => (0, import_rxjs7.fromEvent)(document, "scroll").pipe((0, import_operators5.takeUntil)(this._destroyed)).subscribe(observer)));
  }
  measureBoundingClientRectWithScrollOffset(from) {
    return this.getElementRef().nativeElement.getBoundingClientRect()[from];
  }
...
var CloseScrollStrategy = class {
  ...
  enable() {
    ...
    const stream = this._scrollDispatcher.scrolled(0).pipe((0, import_operators6.filter)((scrollable) => {
...
var OverlayRef = class {
  constructor(_portalOutlet, _host, _pane, _config, _ngZone, _keyboardDispatcher, _document, _location, _outsideClickDispatcher, _animationsDisabled = false, _injector) {
    ...
    this._backdropClick = new import_rxjs8.Subject();
    this._attachments = new import_rxjs8.Subject();
    this._detachments = new import_rxjs8.Subject();
    this._locationChanges = import_rxjs8.Subscription.EMPTY;
    this._backdropClickHandler = (event) => this._backdropClick.next(event);
    this._backdropTransitionendHandler = (event) => {
      this._disposeBackdrop(event.target);
    };
    this._keydownEvents = new import_rxjs8.Subject();
    this._outsidePointerEvents = new import_rxjs8.Subject();
    this._renders = new import_rxjs8.Subject();
  ...
  _detachContentWhenEmpty() {
    this._ngZone.runOutsideAngular(() => {
      const subscription = this._renders.pipe((0, import_operators6.takeUntil)((0, import_rxjs8.merge)(this._attachments, this._detachments))).subscribe(() => {
...
var FlexibleConnectedPositionStrategy = class {
  ...
  constructor(connectedTo, _viewportRuler, _document, _platform, _overlayContainer) {
    ...
    this._positionChanges = new import_rxjs8.Subject();
    this._resizeSubscription = import_rxjs8.Subscription.EMPTY;
    this._offsetX = 0;
    this._offsetY = 0;
...
var CdkConnectedOverlay = class _CdkConnectedOverlay {
  ...
  constructor(_overlay, templateRef, viewContainerRef, scrollStrategyFactory, _dir) {
    ...
    this._dir = _dir;
    this._backdropSubscription = import_rxjs8.Subscription.EMPTY;
    this._attachSubscription = import_rxjs8.Subscription.EMPTY;
    this._detachSubscription = import_rxjs8.Subscription.EMPTY;
    this._positionSubscription = import_rxjs8.Subscription.EMPTY;
  ...
  _attachOverlay() {
    ...
    if (this.positionChange.observers.length > 0) {
      this._positionSubscription = this._position.positionChanges.pipe((0, import_operators6.takeWhile)(() => this.positionChange.observers.length > 0)).subscribe((position) => {
...
var MatSnackBarRef = class {
  constructor(containerInstance, _overlayRef) {
    ...
    this._afterDismissed = new import_rxjs9.Subject();
    this._afterOpened = new import_rxjs9.Subject();
    this._onAction = new import_rxjs9.Subject();
...
var MatSnackBarContainer = class _MatSnackBarContainer extends BasePortalOutlet {
  constructor(_ngZone, _elementRef, _changeDetectorRef, _platform, snackBarConfig) {
    ...
    this._onAnnounce = new import_rxjs9.Subject();
    this._onExit = new import_rxjs9.Subject();
    this._onEnter = new import_rxjs9.Subject();
...
var MatSnackBar = class _MatSnackBar {
  ...
  _attach(content, userConfig) {
    ...
    this._breakpointObserver.observe(Breakpoints.HandsetPortrait).pipe((0, import_operators7.takeUntil)(overlayRef.detachments())).subscribe((state2) => {
