...
var GoogleLoginProvider = class _GoogleLoginProvider extends BaseLoginProvider {
  ...
  constructor(clientId, initOptions) {
    ...
    this._socialUser = new import_rxjs.BehaviorSubject(null);
    this._accessToken = new import_rxjs.BehaviorSubject(null);
    ...
    this._socialUser.pipe((0, import_operators.skip)(1)).subscribe(this.changeUser);
    this._accessToken.pipe((0, import_operators.skip)(1)).subscribe(this._receivedAccessToken);
  }
      ...
      try {
        this.loadScript(_GoogleLoginProvider.PROVIDER_ID, this.getGoogleLoginScriptSrc(lang), () => {
          ...
          if (this.initOptions.oneTapEnabled) {
            this._socialUser.pipe((0, import_operators.filter)((user) => user === null)).subscribe(() => google.accounts.id.prompt(console.debug));
          }
  ...
  getAccessToken() {
    return new Promise((resolve, reject) => {
      if (!this._tokenClient) {
        ...
        this._receivedAccessToken.pipe((0, import_operators.take)(1)).subscribe(resolve);
      }
    });
  }
  revokeAccessToken() {
...
var SocialAuthService = class _SocialAuthService {
  ...
  constructor(config, _ngZone, _injector) {
    ...
    this.lang = "";
    this._user = null;
    this._authState = new import_rxjs.ReplaySubject(1);
    this.initialized = false;
    this._initState = new import_rxjs.AsyncSubject();
  ...
  initialize(config) {
    ...
    Promise.all(Array.from(this.providers.values()).map((provider) => provider.initialize(this.autoLogin, this.lang))).then(() => {
      ...
      this.providers.forEach((provider, key) => {
        if ((0, import_rxjs.isObservable)(provider.changeUser)) {
...
var GoogleSigninButtonDirective = class _GoogleSigninButtonDirective {
  constructor(el, socialAuthService) {
    ...
    this.width = 0;
    this.locale = "";
    socialAuthService.initState.pipe((0, import_operators.take)(1)).subscribe(() => {
